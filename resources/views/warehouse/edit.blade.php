...
@section('page-title', 'تعديل بيانات المستودع')
    ...
    <div class="col-xl-8">
        <div class="card">
            <div class="card-header">
                ...
                تعديل بيانات: <strong>{{ $warehouse->warehouse_name }}</strong>
            </div>
            <div class="card-body p-4">

                <form action="{{ route('warehouse.update', $warehouse->warehouse_id) }}"
                                ...
                                <select name="manager_id" class="form-select">
                                    ...
                                    @foreach($managers as $manager)
                                        <option value="{{ $manager->personal_id }}"
                                            {{ old('manager_id', $warehouse->manager_id) == $manager->personal_id ? 'selected' : '' }}>
                                            {{ $manager->full_name }}
                                        </option>
                                    @endforeach
                                </select>
