...
<div class="sidebar">
    <div class="sidebar-header">
        ...
        <small>مرحباً، {{ session('user.name') }}</small>
    </div>

    <div class="nav-section">القائمة الرئيسية</div>
    <a href="{{ route('marketing.dashboard') }}" class="nav-link active">
        <i class="fas fa-home"></i> لوحة التحكم
    </a>

    <div class="nav-section">إدارة الزبائن</div>
    <a href="{{ route('marketing.customers.index') }}" class="nav-link">
        <i class="fas fa-users"></i> قائمة الزبائن
    </a>
    <a href="{{ route('marketing.customers.create') }}" class="nav-link">
        <i class="fas fa-user-plus"></i> إضافة زبون
    </a>

    @if(session('user.role') === 'manager')
    <div class="nav-section">العروض والهدايا</div>
    <a href="{{ route('marketing.offers.index') }}" class="nav-link">
        <i class="fas fa-gift"></i> العروض
    </a>
    <a href="{{ route('marketing.offers.create') }}" class="nav-link">
    ...
    <div class="row g-4">
        ...
        <div class="col-md-4">
            <div class="stat-card">
                ...
                <div class="stat-number">{{ $vipCustomers ?? 0 }}</div>
        ...
        <div class="col-md-4">
            <div class="stat-card">
                ...
                <div class="stat-number">{{ session('user.role') === 'manager' ? 'مدير' : 'موظف' }}</div>
        ...
        <div class="col-md-6">
            <div class="stat-card text-start">
                ...
                <div class="d-flex flex-column gap-2">
                    <a href="{{ route('marketing.customers.create') }}" class="btn btn-success rounded-3">
                        <i class="fas fa-user-plus me-2"></i> إضافة زبون
                    </a>
                    @if(session('user.role') === 'manager')
                    <a href="{{ route('marketing.offers.create') }}" class="btn btn-outline-success rounded-3">
                        <i class="fas fa-gift me-2"></i> إضافة عرض
                    </a>
                    @endif
                    <a href="{{ route('marketing.customers.index') }}" class="btn btn-outline-secondary rounded-3">
            ...
            <div class="stat-card text-start">
                ...
                <table class="table table-borderless mb-0" style="font-size:0.9rem;">
                    <tr>
                        ...
                        <td><strong>{{ session('user.username') }}</strong></td>
                    </tr>
                    <tr>
