...
<div class="form-card">
    ...
    <form method="POST" action="{{ route('marketing.offers.update', $offer->offer_id) }}">
        @csrf
        @method('PUT')
        ...
        <div class="mb-3">
            ...
            <textarea name="offer_details" class="form-control" rows="3">{{ old('offer_details', $offer->offer_details) }}</textarea>
        </div>
        ...
        <div class="mb-3">
            ...
            <select name="status" class="form-control">
                <option value="نشط" {{ $offer->status == 'نشط' ? 'selected' : '' }}>نشط</option>
                <option value="منتهي" {{ $offer->status == 'منتهي' ? 'selected' : '' }}>منتهي</option>
            </select>
        </div>

        <div class="text-center mt-4">
            <button type="submit" class="btn-submit px-5 py-2" id="updateBtn">💾 تحديث العرض</button>
            <a href="{{ route('marketing.offers.index') }}" class="btn-back px-4 py-2" id="cancelBtn">🔙 إلغاء</a>
        </div>
    </form>
</div>
