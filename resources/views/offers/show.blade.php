...
<div class="top-controls">
    ...
    <a href="{{ route('dashboard') }}" class="btn-back" style="text-decoration:none;"><i class="bi bi-house-door"></i> Dashboard</a>
</div>
...
<div class="container">
    <div class="card">
        ...
        <div class="card-body">
            <div class="row">
                <div class="col-md-6"><div class="info-row"><span class="info-label" id="nameLabel">اسم العرض:</span><span class="info-value">{{ $offer->offer_name }}</span></div></div>
                <div class="col-md-6"><div class="info-row"><span class="info-label" id="detailsLabel">التفاصيل:</span><span class="info-value">{{ $offer->offer_details ?? '-' }}</span></div></div>
                <div class="col-md-6"><div class="info-row"><span class="info-label" id="startLabel">تاريخ البداية:</span><span class="info-value">{{ $offer->start_date ? \Carbon\Carbon::parse($offer->start_date)->format('d/m/Y') : '-' }}</span></div></div>
                <div class="col-md-6"><div class="info-row"><span class="info-label" id="endLabel">تاريخ النهاية:</span><span class="info-value">{{ $offer->end_date ? \Carbon\Carbon::parse($offer->end_date)->format('d/m/Y') : '-' }}</span></div></div>
                <div class="col-md-6"><div class="info-row"><span class="info-label" id="statusLabel">الحالة:</span><span class="info-value">{{ $offer->status ?? 'نشط' }}</span></div></div>
            </div>
        </div>
        ...
        @if(session('user.role') === 'manager')
            <div class="card mt-3" style="margin: 20px;">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('marketing.offers.assign.store', $offer->offer_id) }}">
                        @csrf
                        @forelse($vipCustomers as $customer)
                            <div class="form-check">
                                <input type="checkbox" name="customer_ids[]" value="{{ $customer->personal_id }}" class="form-check-input" id="cust{{ $customer->personal_id }}"
                                    {{ $offer->customers->contains($customer->personal_id) ? 'checked' : '' }}>
                                <label class="form-check-label" for="cust{{ $customer->personal_id }}">{{ $customer->firstNmae }} {{ $customer->lastName }} ({{ $customer->email }})</label>
                            </div>
                        @empty
        ...
        <div class="card-footer text-center">
            <a href="{{ route('marketing.offers.index') }}" class="btn-back"><i class="fas fa-arrow-right"></i> <span id="backText">العودة إلى القائمة</span></a>
        </div>
    </div>
</div>
